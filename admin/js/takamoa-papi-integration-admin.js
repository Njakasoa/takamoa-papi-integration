		...
		function onScan(decodedText) {
			...
			$.post(takamoaAjax.ajaxurl, {
				...
				.done(function (res) {
					if (res.success) {
						var html =
							'<div class="tk-result"><div class="tk-title">' +
							res.data.name +
							'</div><div class="tk-sub">Email: ' +
							(res.data.email || '—') +
							'<br>Téléphone: ' +
							(res.data.phone || '—') +
							'<br>Entreprise: ' +
							(res.data.description || '—') +
							'<br>Status: <span class="ticket-status">' +
							res.data.status +
							'</span></div>';
						if (res.data.status === 'GENERATED') {
							html +=
								'<button id="validate-ticket" data-ref="' +
								decodedText +
								'" class="tk-btn primary">Valider le billet</button>';
						}
						html += '</div>';
						scanResult.html(html);
					} else {
						scanResult.html(
							'<div class="tk-sub tk-bad">' +
							(res.data && res.data.message
								? res.data.message
								: 'Billet introuvable') +
							'</div>'
